...
export const EXPERIMENTS: Experiment[] = [
  ...
  },
  {
    ...
    expected_result:
      'With independent NEAREST quantizers the pair still cancels exactly: ' +
      'e_pair_digital == 0 on every cycle except exact half-LSB ties (none at ' +
      'N=3.13; measured 0/512 nonzero). Genuine divergence needs DSM ' +
  ...
  },
  {
    ...
    expected_result:
      "Distinct DSM states ('dsm_inj' seeding, spec section 7): " +
      'e_pair_digital jumps in {-1, 0, +1} LSB, nonzero on a sustained ' +
      '~64 percent of cycles (measured 326/512). Not recommended.',
  },
  {
    id: 'exp06',
  ...
  },
  {
    ...
    expected_result:
      'Ten-level periodic sawtooth (period 10) spanning the full +-0.5 LSB ' +
      'range, mean ~ +0.05 LSB (measured +0.047 LSB over 512 cycles; FP ' +
      'half-LSB ties can flip a +0.5 to -0.5). Deterministic spur, not noise.',
  },
  {
    id: 'exp08',
    ...
    expected_result:
      'mean(e_FB_abs) -> 0 (measured -0.002 LSB); multi-level shaped error ' +
      'in 0.1-LSB steps with instantaneous peaks +-0.7 LSB (noise shaping ' +
      'trades DC for peaks; spec section 6 Test 4).',
  },
  {
    id: 'exp09',
    ...
    expected_result:
      'Mode D: e_pair_digital == 0 exactly; mode B (distinct DSM state via ' +
      "'dsm_inj'): e_pair_digital != 0 on a sustained ~64 percent of cycles " +
  ...
  },
  {
    ...
    expected_result:
      'Phase error wrapCycles(-L*alpha) = -0.13 cycle (|e| = 46.8 deg, 66.6x half-LSB).',
  },
  {
    id: 'exp11',
  ...
  },
  {
    ...
    description:
      'FB DTC gain 1.00, INJ DTC gain 1.01 at off-grid N=3.13 so the ' +
      'injection DTC codes actually sweep (DTC full range T_vco/4, ~20 ps).',
    config: mk({ n_div: 3.13, dtc_fb_gain: 1.0, dtc_inj_gain: 1.01 }),
    expected_result:
      'Code-dependent gain error appears: e_pair_analog up to 96.7 fs ' +
      '(0.01 * 31/256 cycle at the naive-mapping max code c_INJ=31), and ' +
      'max|e_ZC_hw| = 205.9 fs (quantization <= 156.5 fs + gain error). ' +
      'Full-range bound remains ~200 fs = 0.64 LSB (Test 7).',
  },
  {
    id: 'exp14',
    name_zh: 'DTC 正弦 INL',
  ...
  },
  {
    ...
    description:
      'Off-grid N in {3.01, 3.06, 3.11, 3.16, 3.21, 3.24} (12.04-12.96 GHz) ' +
      'so DTC codes sweep every cycle, each with normalized vs fixed-time ' +
      '(312.5 fs) DTC LSB.',
    configs: [
      mk({ n_div: 3.01, dtc_mode: 'normalized' }),
      mk({ n_div: 3.01, dtc_mode: 'fixed_time' }),
      mk({ n_div: 3.06, dtc_mode: 'normalized' }),
      mk({ n_div: 3.06, dtc_mode: 'fixed_time' }),
      mk({ n_div: 3.11, dtc_mode: 'normalized' }),
      mk({ n_div: 3.11, dtc_mode: 'fixed_time' }),
      mk({ n_div: 3.16, dtc_mode: 'normalized' }),
      mk({ n_div: 3.16, dtc_mode: 'fixed_time' }),
      mk({ n_div: 3.21, dtc_mode: 'normalized' }),
      mk({ n_div: 3.21, dtc_mode: 'fixed_time' }),
      mk({ n_div: 3.24, dtc_mode: 'normalized' }),
      mk({ n_div: 3.24, dtc_mode: 'fixed_time' }),
    ],
    expected_result:
      'All points off-grid: max|e_FB_abs| 144.7-155.7 fs (nonzero ' +
      'everywhere). normalized: phase LSB constant 1.40625 deg, e_ZC_hw = ' +
      'quantization only (<= half-LSB); fixed_time: 312.5-fs LSB != ' +
      'T_vco/256 away from 12.5 GHz, adding code-dependent scaling error, ' +
      'e.g. N=3.01: max|e_ZC_hw| 155.7 -> 489.0 fs and max|e_pair_analog| ' +
      '0 -> 764.1 fs (spec section 11).',
  },
  {
    id: 'exp19',
  ...
  },
  {
    ...
    description:
      'N=3.13, ef1, sin injection (K=0.4, 1 MHz detuning, ' +
    ...
    expected_result:
      '(a) full: 512/512 fire, |e_ZC_hw| <= 0.00297 cycle, residual ' +
      'theta_plus rms (last 256) 0.015 rad. (b) dsm_only ungated: e_ZC_hw ' +
      'sweeps the full +-0.5 cycle (rms 0.289), kicks land at arbitrary ' +
      'pulse offsets and unlock the loop: theta_plus rms (last 256) 1.81 ' +
      'rad. (c) dsm_only gated at 0.0625 cycle: fires 67/512 (13%), ' +
      'theta_plus rms (last 256) 0.102 rad — gating restores a bounded ' +
      'lock at a reduced correction rate (all measured, seed 12345).',
  },
  {
    id: 'exp22',
    ...
    description:
      'N=3.13, mode D, full actuator, three configs differing only in the ' +
      'quantizer order: ef1 (1st-order error feedback) vs mash11 vs mash111.',
    configs: [
      mk({ n_div: 3.13, arch_mode: 'D', quantizer: 'ef1' }),
      mk({ n_div: 3.13, arch_mode: 'D', quantizer: 'mash11' }),
      mk({ n_div: 3.13, arch_mode: 'D', quantizer: 'mash111' }),
    ],
    expected_result:
      'Higher order spreads the instantaneous error (measured, 512 cycles): ' +
      'e_FB_abs rms/peak 0.466/0.76 LSB (ef1), 0.686/1.36 LSB (mash11), ' +
      '1.194/1.76 LSB (mash111); e_ZC_total rms 0.00182/0.00268/0.00467 ' +
      'cycle respectively. n_int at N=3.13 stays {3,4} for all three; ' +
